package com.dsankovsky.translatorappkmm.android.voice_to_text.data
...
import com.dsankovsky.translatorappkmm.core.domain.utils.toCommonStateFlow
import com.dsankovsky.translatorappkmm.voice_to_text.domain.VoiceToTextParser
import com.dsankovsky.translatorappkmm.voice_to_text.domain.VoiceToTextParserState
...
class AndroidVoiceToTextParser(
    ...
    override fun startListening(languageCode: String) {
        ...
        if (!SpeechRecognizer.isRecognitionAvailable(application)) {
            _state.update {
                it.copy(
                    error = application.getString(R.string.speech_recognition_in_not_available)
                )
            }
            return
        }
    ...
    override fun onRmsChanged(rsmDb: Float) {
        _state.update {
            it.copy(
                powerRatio = rsmDb * (1f / (12f - (-2f))) // now value in [0: 1]
            )
        }
    }
